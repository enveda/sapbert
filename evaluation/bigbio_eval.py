...
def parse_args_bigbio():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["medmentions_full", "medmentions_st21pv", "bc5cdr", 'nlmchem','nlm_gene','gnormplus','ncbi_disease'],
    ...
    parser.add_argument('--abbreviations_path', type=str, help='Path to .json file of abbreviations in each article')
...
def sapbert_predict(
    ...
    if dict_cache_filepath is not None:
        if os.path.isfile(dict_cache_filepath):
            dict_dense_embeds = torch.load(dict_cache_filepath)
        else:
            ...
            torch.save(dict_dense_embeds, dict_cache_filepath)
    else:
        dict_dense_embeds = model_wrapper.embed_dense(
            names=dict_names,
            show_progress=True,
